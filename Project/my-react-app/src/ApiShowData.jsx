        ...
        state.data.map((a,index)=>{
          ...
          let inCart=state.cartData.find(item=>item.id===a.id)
            ...
            return(<div id='card'  >
                ...
                {
                  inCart?(<div> <button onClick={()=>dispatch({type:"increment" ,payload:a.id})}>+</button> {inCart.quantity} <button onClick={()=>dispatch({type:"decrement" ,payload:a.id})}>-</button></div>):(<button onClick={()=>{dispatch({type:"addToCart",payload:a})}}>click</button>)
                }
